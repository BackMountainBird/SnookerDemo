...
void PauseState::enter()
{
	...
	OgreFramework::getSingletonPtr()->m_pTrayMgr->createButton(OgreBites::TL_CENTER, "OptionBtn",     "Options",250);
    OgreFramework::getSingletonPtr()->m_pTrayMgr->createButton(OgreBites::TL_CENTER, "BackToMenuBtn", "Return to Menu", 250);
    OgreFramework::getSingletonPtr()->m_pTrayMgr->createButton(OgreBites::TL_CENTER, "ExitBtn", "Exit SnookerDemo", 250);
...
void PauseState::buttonHit(OgreBites::Button *button)
{
	...
	else if(button->getName() == "OptionBtn"){
		//popAppState();
		pushAppState(findByName("OptionState"));
	}
		//popAllAndPushAppState(findByName("OptionState"));
}
