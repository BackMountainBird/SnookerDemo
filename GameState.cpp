...
void GameState::enter()
{
    ...
    m_pCamera->setPosition(Vector3(0, 0, 4000));
    m_pCamera->lookAt(Vector3(0, 0, -1));
    ...
    m_pCurrentObject = 0;
	m_pGameCore= reinterpret_cast<Snooker *>(new Snooker());
    OgreFramework::getSingletonPtr()->m_pSoundManager=OgreOggSound::OgreOggSoundManager::getSingletonPtr();
	OgreFramework::getSingletonPtr()->m_pSoundManager
		->init("",100U,64U,m_pSceneMgr);
	m_pSceneMgr->getRootSceneNode()->createChildSceneNode("SceneNode_Sound");
	//->attachObject(OgreFramework::getSingletonPtr()->m_pSoundManager->getListener());
...
{
	m_pGameCore->beginGame(m_pSceneMgr);
}
...
bool GameState::keyPressed(const OIS::KeyEvent &keyEventRef)
{
	...
	if (!m_bSettingsMode)
	{
		m_pGameCore->keyPress(keyEventRef,OgreFramework::getSingletonPtr()->m_pKeyboard);
	}
	//#endif // _DEBUG
    return true;
}
...
{
	m_pSceneMgr->getSceneNode("SceneNode_Sound")->setPosition(m_pCamera->getPosition());
	OgreFramework::getSingletonPtr()->m_pSoundManager->update(timeSinceLastFrame);
    m_FrameEvent.timeSinceLastFrame = timeSinceLastFrame;
    OgreFramework::getSingletonPtr()->m_pTrayMgr->frameRenderingQueued(m_FrameEvent);
#if OGRE_PLATFORM == OGRE_PLATFORM_WIN32
	int i=m_pGameCore->UPDATE_STEP;
	while(i)
	{
		m_pGameCore->update(Ogre::Real(timeSinceLastFrame/1000/(i--)));
	}
#else
	while(i)
	{
		m_pGameCore->update(Ogre::Real(timeSinceLastFrame/(i--)));
	}
#endif
    if(m_bQuit == true)
    {
    ...
    {
		Ogre::Vector3 te=m_pSceneMgr->getEntity("Entity_Blue")->getParentSceneNode()->getPosition();
        if(m_pDetailsPanel->isVisible())
        {
			...
			m_pDetailsPanel->setParamValue(8, Ogre::StringConverter::toString(te.x));
			m_pDetailsPanel->setParamValue(9, Ogre::StringConverter::toString(te.y));
			m_pDetailsPanel->setParamValue(10, Ogre::StringConverter::toString(te.z));
        }
    }
...
void GameState::buildGUI()
{
	...
	items.push_back("Blue.x");
	items.push_back("Blue.y");
	items.push_back("Blue.z");
